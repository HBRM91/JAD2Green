...
CREATE TABLE IF NOT EXISTS project_intensity_config (
    ...
    denominator_code     TEXT NOT NULL REFERENCES intensity_denominators(code),
    denominator_value    NUMERIC(20,4) NOT NULL CHECK (denominator_value > 0),
    note                 TEXT,
    created_at           TIMESTAMPTZ DEFAULT now(),
    UNIQUE (project_id, denominator_code)
);
...
VALUES
    ...
    -- Charbon: kg → MJ (charbon vapeur standard)
    ('tonne_charbon', 'TEP', 0.727, 'energy_content', 'AMEE Maroc/AIE 2023', '2020-01-01'),
    -- Gaz naturel: m3 → kg (densité standard 0°C 1atm)
    ('m3_gaz_naturel', 'kg', 0.717, 'density', 'CODIGAZ/GRTgaz', '2020-01-01'),
    -- kWh_th (thermique) → MJ
    ('kWh_th', 'MJ', 3.6, 'exact', 'SI', '2000-01-01'),
    -- tonne → kg
    ('tonne', 'kg', 1000.0, 'exact', 'SI', '2000-01-01'),
